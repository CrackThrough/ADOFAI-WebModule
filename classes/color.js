...
class Color {
    ...
    toString() {
        function clampAndPad(val) {
            Math.max(0, Math.min(Math.floor(val), 255)).toString(16).padStart(2, '0');
        }

        let result = clampAndPad(this.R) + clampAndPad(this.G) + clampAndPad(this.B);
    ...
    static FromNumber(num) {
        ...
        num = Math.abs(num ?? 0);
        let _c;
        if (num >= 2 ** 24 && num < 2 ** 32) { // includes alpha value
